...
/*   Updated: 2021/01/18 13:01:39 by mochegri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void     ft_horis_interst(t_game *game, t_ray *ray)
{
	...
	while (ft_is_in_map(ray->h_end_p, game))
	{
		...
		else
		{
			...
			printf("(%f,%f)",ray->h_end_p.x, ray->h_end_p.y);
		}
		printf("h %d\n", i++);
	}
}
...
void	ft_verti_intersect(t_game *game, t_ray *ray)
{
    ...
    * tan(ray->angl);
	...
	ray->h_y_steps = tile_size * tan(ray->angl);
	...
	ray->h_end_p.y -= (ray->is_facing & LEFT) ? 1 : 0;
...
{
    if (p.x < 0 || p.y < 0 || p.x > game->cube->resolution[0]
    || p.y > game->cube->resolution[1])
        return (0);
    return (1);
}
